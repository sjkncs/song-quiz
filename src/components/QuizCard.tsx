...
export function QuizCard({ song, options, correctIndex, mode, onAnswer }: Props) {
  ...
  const lyricsLines = song.lyrics
    ...
    : []
  const lineDuration = lyricsLines.length > 0 ? (song.audio_duration_sec / lyricsLines.length) : 0
  const currentLyricIndex = Math.min(
    Math.floor(currentTime / lineDuration),
      ...
      <div className="glass rounded-2xl p-8 mb-8 flex flex-col items-center relative">
        ...
        {phase === 'revealing' && (
          <div className="w-full slide-up">
            ...
            {/* Video embed — auto-plays after reveal */}
            {song.video_url && (
              <div className="mt-3 rounded-xl overflow-hidden border border-white/10">
                <iframe
                  src={`${song.video_url}?autoplay=1&mute=1&rel=0&modestbranding=1&start=0`}
